...
//  WeiGuo
//
//  Created by YY on 2032.5/1/16.
//  Copyright © 2032.5 WeiGuo. All rights reserved.
//
...
#define RADIUS  17.5
#define CenterY_item  Height_StatusBar == 20 ? 22.5 : 32.5

@interface WGTabBarView()<CAAnimationDelegate>
{
...
- (void)setDefColor:(UIColor *)defColor{
    ...
    _defColor = defColor;
    for (WGTabBarItem * tempItem in _tabBarItems) {
...
- (void)selectItem:(UITapGestureRecognizer *)tap{
    ...
    WGTabBarItem *tapItem = (WGTabBarItem *)tap.view;
...
- (void)setBePath:(NSInteger) index{
    ...
    CGPoint center = CGPointMake(x,CenterY_item);
...
- (void)pathAnimation:(NSInteger)formIndex toIndex:(NSInteger)toIndex{
    ...
    CGPoint center1 = CGPointMake(x1,CenterY_item);
    ///目标中心点
    CGFloat x2 = K_WIDTH_QW/count*(toIndex+0.5);
    CGPoint center2 = CGPointMake(x2,CenterY_item);
    /// 相隔几个item
...
- (void)setBadge:(NSInteger)count1 index:(NSUInteger)index{
    WGTabBarBadge *badgeLab = [self viewWithTag:BADGETAG + index];
    if(index>_tabBarItems.count){
        NSLog(@"设置角标  下标越界啦");
        return;
    }
    if(!badgeLab){
        badgeLab = [[WGTabBarBadge alloc] init];
